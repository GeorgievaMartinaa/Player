...
public class PlayerServiceImpl implements PlayerService {
    ...
    public Player vote(Long id) {
        Player player = findById(id);
        Integer vote = player.getVotes();
        vote= vote+1;
        player.setVotes(vote);
        playerRepository.save(player);
        return player;
    }
    ...
    public List<Player> listPlayersWithPointsLessThanAndPosition(Double pointsPerGame, PlayerPosition position) {
        if(pointsPerGame != null && position != null)
            return playerRepository.findAllByPointsPerGameLessThanAndPositionLike(pointsPerGame,position);
            ...
            return playerRepository.findAllByPositionLike(position);
    }
}
